...
//아래의 클래스를 쓰레드로 선언하는 순간부터, 이 인스턴스들은 서로 비동기적으로 동작할 수 있다.
public class MessageThread extends Thread {
	...
	public void send(String msg) {
		try {
			//백터에 들어있는 메세지쓰렏 만큼 반복문 수행, write,flush 수행
			for(int i=0;i<multiServer.clientList.size();i++) {
				MessageThread messageThread=multiServer.clientList.get(i);//백ㅌ에 들어있는 메세지 쓰레드를 하나씩 꺼낸다
