...
import day1111.member.LoginForm;
import day1111.member.RegistForm;//다른 패키지에 있기때문에 import필요

/*
 * BoardApp가 모든 페이지들을 보유하고 있는 최상위 컨테이너 이므로
 * 각 페이지마다 서로 공유할 데이터가 있다면 BoardApp에 데이터를 두고 처리하면 됨
 * */
public class BoardApp extends JFrame {
	JPanel p_north, p_center;
	JButton bt_board, bt_regist, bt_login;

	private JPanel[] pages = new JPanel[5];// 화면 전환에 사용 될 패널들을 담게 될 배열

	// 상수란? 변하지 않는 데이터에 의미를 부여하여 직관성을 높인다
	...
	// 모든 페이지들이 사용 할 접속정보 객체를 공통으로 선언
	// 이 커넥션 객체는 프로그램이 가동과 동시에 접속을 얻어다 놓자
	// 접속이 성공되었을때 접속 정보를 가진 객체를 Connection 객체라한다
	// 따라서 이 객체의 인스턴스가 만약 null이라면 접속정보 자체가 없으므로 접속이 실패한 것으로 판단한다
	private Connection con;

	// 로그인 상태 여부를 보관할 변수
	private boolean hasSession = false;// 세션을보관하고있는지

	// 회원정보를 보관할 자료형
	private BoardMember boardMember; // 이 변수에 데이터가 채워지는 시점 ->로그인 성공시 인스턴스 생성

	public BoardApp() {
		this.getConnection();// 프레임을 보여주기 직전에 데이터베이스 접속 성고해놓자
		// 생성
		p_north = new JPanel();
		bt_board = new JButton(ImageUtil.getIcon(this.getClass(), "res/board/board.png", 100, 45));
		bt_regist = new JButton(ImageUtil.getIcon(this.getClass(), "res/board/regist.png", 100, 45));
		bt_login = new JButton(ImageUtil.getIcon(this.getClass(), "res/board/login.png", 100, 45));
		p_center = new JPanel();
		pages[0] = new BoardList(this);// 게시판 목록페이지
		pages[1] = new BoardWrite(this);// 게시판 글쓰기 페이지
		pages[2] = new BoardDetail(this);// 게시판 상세보기 페이지
		pages[3] = new RegistForm(this);// 회원가입
		...
		p_center.add(pages[1]);// 중앙 패널에 게시판 등록
		p_center.add(pages[2]);// 중앙 패널에 상세보기
		p_center.add(pages[3]);// 회원가입
		p_center.add(pages[4]);// 로그인
		...
		// 이벤트 구현시, 재정의할 메서드가 달랑 1개인 경우 굳이 내부익명 클래스라는 표기를
	...
	public void loginCheck(int page) {
		if (hasSession == false) {// 로그인 하지 않은 상태
			JOptionPane.showMessageDialog(this, "로그인 하시오");
	...
	// 접속을 해제하는 메서드 정의
	// 이 메서드는 윈도우창을 닫을 때 호출 될 예정
	public void disConnection() {
		if (con != null) {// null이 아닐때만 닫아야 함, 만약 이런 확일절차를 거치지 않으면 nullpointException발생
			try {
				con.close();
			} catch (SQLException e) {
				e.printStackTrace();
