...
public class BoardList extends JPanel {
	...
	public BoardList(BoardApp boardApp) {
		...
		table.addMouseListener(new MouseAdapter() {
			public void mouseReleased(MouseEvent e) {
				...
				boardDetail.updateHit(Integer.parseInt(board_id));//클릭했을때 조회수 증가
	...
	public void getList() {
		...
		try {
			...
			//스크롤이 가능하고(next()능력에 자유자재로 이동능력),읽기 전용의 rs를 만들기 위한 옵션
			
			rs=pstmt.executeQuery();
			rs.last();
			int total=rs.getRow();//총 레코드 수 반환
			
			//rs에 들어있는 데이터를 이차원 배열로 옮겨심자
			//그러기 위해서는 먼저 이차원배열을 준비해놓자
			String[][] records=new String[total][5];
			rs.beforeFirst();
			int index=0;
			while(rs.next()) {//커서한칸전진
