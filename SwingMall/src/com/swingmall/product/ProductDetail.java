...
public class ProductDetail extends Page{
	...
	JButton bt_buy;//구매하기버튼
	JButton bt_cart;//장바구니버튼
	...
	public ProductDetail(ShopMain shopMain) {
		...
		bt_cart.addActionListener((e)->{
			...
			//장바구니에 정보가 담겼다고 알려주고, 장바구니 이도 여부를 확인해야 함
	...
	//상세페이지를 보여질때 데이터를 채워넣는 메서드(생성자에서 하면 디자인 처리에 타이밍 적으로 제한이 많다)
	public void init(ProductVO vo,Image img) {
		this.vo=vo;//멤버변수에 현재 보고있는 상품vo주입
		la_brand.setText(vo.getBrand());//브랜드채워넣기
	...
	//장바구니에 등록(DB로 보관하지 않고, 오직 메모리상으로 저장 할 예정)
	public void registCart() {
		...
		cartVO.setSize(ch_size.getSelectedItem());//사이즈
		cartVO.setEa(1);//장바구니에 담을때는 기본이 1개
		
		cartPage.addCart(cartVO);//장바구니에 상품1건 추가
